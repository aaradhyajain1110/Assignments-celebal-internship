# -*- coding: utf-8 -*-
"""week1_<Aaradhya Jain>.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1M-cRO7Gs3fvNslXUjiEBrZgvsGpRzj69
...
dept_stats = (
    ...
    .agg({"salary": "mean", "performance": "mean"})
...
**Eigen vector** - An eigenvector is a special direction that the matrix does not rotate.

When you apply the matrix to an eigenvector, something very simple happens:

The vector stays on the same line (same direction)
It may only get longer or shorter

**Eigen Value** - The eigenvalue tells you how much stretching or squishing happens:

λ > 1 → vector gets stretched (longer)
0 < λ < 1 → vector gets squished (shorter)
λ < 0 → vector flips direction + scales
λ = 1 → no change in length
λ = 0 → it collapses to zero

**Stretch or Squish**
When a matrix “stretches” or “squishes” a vector, it means the matrix is changing the vector’s length when it transforms it.

Stretching → the vector becomes longer (it’s scaled up)
Squishing → the vector becomes shorter (it’s scaled down)

The direction might also change in general, but for special vectors (eigenvectors), only the length changes.
...
Think of this as the “output directions” after transformation. It lives in the data space of rows.
S (singular values)
These are just numbers (like strengths). They tell how important each direction is — bigger value = more information/variance.
Vᵀ (right singular vectors)
These are the input directions. They define the principal axes in feature space.
PCA and SVD are very closely related:PCA finds directions where data varies the most.

In SVD: V (or Vᵀ) gives those directions.V (from Vᵀ) contains the directions of maximum variance.First column of V → direction of highest variance (1st principal component),Second column → next highest, and so on
...
iqr_s    = np.percentile(salary, 75) - np.percentile(salary, 25) # YOUR CODE HERE — use np.percentile
...
# Histogram + KDE
fig, ax = plt.subplots()
sns.histplot(salary, kde=True, ax=ax)
# YOUR CODE HERE
...
- **Population:** The entire group of individuals or data you want to study.
- **Sample:** A smaller subset taken from the population to analyze and make conclusions.
- **Descriptive statistic:** A numerical summary that describes or shows features of a dataset (like mean, median, or standard deviation).
- **Inferential statistic:** Methods used to make predictions or draw conclusions about a population based on a sample.
...
t_stat, p_value = stats.ttest_1samp(eng_salaries, overall_mean)
 # YOUR CODE HERE — use stats.ttest_1samp
...
# Pearson correlation
r, r_pval = stats.pearsonr(df_filled["salary"], df_filled["years_exp"])  # YOUR CODE HERE — use stats.pearsonr
print(f"\nPearson r (salary vs years_exp): {r:.4f}, p={r_pval:.4f}")

"""**H₀ / H₁ and Interpretation (5.2):**

- H₀: Null Hypothesis Engineering salaries are equal to the overall company average salary.
- H₁: Alternative Hypothesis Engineering salaries are significantly higher than the overall company average salary.
- Conclusion: If the p-value is less than 0.05, we reject H₀ and conclude that Engineering salaries are significantly higher than the company average. Otherwise, we fail to reject H₀ because there is not enough evidence of a significant difference.
...
fig, ax = plt.subplots()
# YOUR CODE HERE
ax.hist(train_dist, bins=30, alpha=0.5, label='Training Distribution')
ax.hist(drift_dist, bins=30, alpha=0.5, label='Production Distribution')
ax.legend(); ax.set_title('Distribution Shift Visualization')
plt.tight_layout(); plt.show()

"""**Concept Drift vs Covariate Drift & PSI Thresholds (5.5):**

- **Concept drift:** The relationship between input data and target output changes over time, causing the model’s predictions to become less accurate.
- **Covariate drift:** The distribution of input features changes, even though the relationship between inputs and outputs may stay the same.
- **PSI < 0.1:** No significant distribution shift (stable data).
- **PSI 0.1–0.2:** Minor/moderate shift detected; monitor the model closely.
- **PSI > 0.2:** Major shift detected; model retraining may be needed.
- **Retraining trigger example:** A large drop in model accuracy or a PSI value greater than 0.2.
...
p_red_then_blue = (4 / 10) * (3 / 9)  # YOUR CODE HERE

# Conditional: P(blue second | red first)
p_blue_given_red = 3 / 9  # YOUR CODE HERE
...
# ── Binomial ────────────────────────────────────────────────────────────────
k = np.arange(0, 21)
axes[1].plot(k, binom.pmf(k, 20, 0.5), marker='o', label='p=0.5')
axes[1].plot(k, binom.pmf(k, 20, 0.7), marker='o', label='p=0.7')
...
# ML use case: ...
axes[2].plot(k2, poisson.pmf(k2, 3), marker='o')
...
|------|---------|-----------------|
| Prior | P(Spam) | Probability that an email is spam before seeing any word = 0.30 |
| Likelihood | P(word \| Spam) | Probability of seeing the word “free” given the email is spam = 0.80 |
| Evidence | P(word) | Overall probability of seeing the word “free” in any email = 0.2750|
| Posterior | P(Spam \| word) | Updated probability that an email is spam after seeing “free” = 0.8727 |
...
sample_means = np.array([
    np.mean(np.random.choice(population, sample_size, replace=True))
...
ax.plot(x, stats.norm.pdf(x, pop_mean, clt_std), label="CLT Normal")
...
ANS-
This result matters because it shows that even if the original data is not normally distributed (like exponential data), the distribution of sample means becomes approximately normal when we take enough samples. This is the core idea behind the Central Limit Theorem, which is fundamental in statistics and machine learning.

The CLT justifies the assumption of normality of sampling distributions, which is used in many classical statistical tests like t-tests, confidence intervals, ANOVA, and regression inference.
